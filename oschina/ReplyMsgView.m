...
- (void)viewDidLoad
{
    ...
    [ToolHelp roundTextView:self.txtReply];
    ...
    {
        [self.webViewReply loadHTMLString:[ToolHelp generateCommentDetail:self.parentComment] baseURL:nil];
        [ToolHelp clearWebViewBackground:self.webViewReply];
    }
...
- (void)viewDidUnload
{
    ...
    [ToolHelp ReleaseWebView:self.webViewReply];
...
- (IBAction)clickReply:(id)sender {
    ...
    {
        [ToolHelp ToastNotification:@"错误 回复内容不能为空" andView:self.view andLoading:NO andIsBottom:NO];
        return;
    }

    MBProgressHUD *hud = [[MBProgressHUD alloc] initWithView:self.view];
    [ToolHelp showHUD:@"正在回复" andView:self.view andHUD:hud];
                                     ...
                                     success:^(AFHTTPRequestOperation *operation, id responseObject) {
                                      ...
                                      switch (error.errorCode) {
                                          ...
                                          case 0:
                                          {
                                              ...
                                              [ToolHelp noticeLogin:self.parentViewController.view andDelegate:self andTitle:[ToolHelp getCommentLoginNoticeByCatalog:self.catalog]];
                                          }
                                              break;
                                          case -2:
                                          case -1:
...
{
    [ToolHelp processLoginNotice:actionSheet andButtonIndex:buttonIndex andNav:self.navigationController andParent:self.parentViewController];
}
#pragma 浏览器链接处理
