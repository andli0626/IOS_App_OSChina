...
{
    [super viewDidLoad];
    [ToolHelp roundTextView:self.txtComment];
    ...
    [[AFOSCClient sharedClient] getPath:url parameters:nil success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        @try {
            ...
            if (root == nil) {
                [ToolHelp ToastNotification:@"加载出现异常" andView:self.view andLoading:NO andIsBottom:NO];
                return ;
            }
            ...
            NSString *pubTime = [NSString stringWithFormat:@"在%@ 更新了动态 %@", self.singleTweet.fromNowOn, [ToolHelp getAppClientString:singleTweet.appClient]];
...
- (void)clickComment:(id)sender
{
    if ([Config Instance].isCookie == NO) {
        ...
        [ToolHelp noticeLogin:self.view andDelegate:self andTitle:@"请先登录后发表评论"];
        return;
    }
    NSString *content = self.txtComment.text;
    if ([content isEqualToString:@""]) {
        [ToolHelp ToastNotification:@"错误 评论内容不能为空" andView:self.view andLoading:NO andIsBottom:NO];
        return;
    }
    ...
    [[AFOSCClient sharedClient] postPath:api_comment_pub parameters:[NSDictionary dictionaryWithObjectsAndKeys:@"3",@"catalog",[NSString stringWithFormat:@"%d", singleTweet._id],@"id",[NSString stringWithFormat:@"%d", [Config Instance].getUID],@"uid",content,@"content",self.switchToZone.isOn ? @"1" : @"0",@"isPostToMyZone", nil] success:^(AFHTTPRequestOperation *operation, id responseObject) {
        ...
        [hud hide:YES];
        [ToolHelp ToastNotification:@"评论发表失败" andView:self.view andLoading:NO andIsBottom:NO];
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType
{
    ...
    {
        //弹出大图
        [ToolHelp pushTweetImgDetail:singleTweet.imgBig andParent:self];
        return NO;
    }
    else
    {
